...
function edit(){
	if(timer==false){
		...
		lastrequest=0;
		timer=true;
		t=setInterval(function(){times+=0.5},500);
		c=setInterval(function(){if(times-lastrequest>=1){
			console.log('Chosen.');
			clearInterval(t);
			clearInterval(c);
			timer=false;
...
function act(step){
	if(step==1){
		...
		$('#btn').html('登出');
		if(confirm('你真的要登出嘛O_o')){console.log('logout.');window.open('?t=out','_self');};
	}else if(step>=7){
		$('#btn').html('取消');
	}
	setTimeout(function(){$('#btn').html('(O_o)?');},1000);
}
function upload(){
...
function submits(){
	...
	if(confirm('你真的要发布/编辑嘛O_o')){
        ...
        data: {title:t,content:c,dat:d,tag:a,ifzd:zd,editn:editnum},
        dataType: "text",
