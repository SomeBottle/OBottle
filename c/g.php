...
if ($type == 'getpage') {
    if ($mode == 'normal') {
        ...
        if (file_exists('./../t/' . $page . '.php') || stripos($page, 'tag') !== false) {
            ...
            /*替换小站名*/
            $c = preg_replace("/\t|\\[intro\\]/", intro(), $c);
            /*替换小站描述*/
            $c = preg_replace("/\t|\\[year\\]/", date('Y'), $c);
            /*替换小站年份*/
            $c = preg_replace("/\t|\\[avatar\\]/", avatar(), $c);
            /*替换小站头像*/
            $c = preg_replace("/\t|\\[host\\]/", host() . '#m', $c);
            /*替换小站链接*/
            preg_match('/\\[css:(.*?)\\]/i', $c, $match);
            $c = preg_replace('/\\[css:(.*?)\\]/i', './t/' . $match[1], $c);
            /*替换CSS路径*/
            preg_match('/\\[js:(.*?)\\]/i', $c, $match2);
            $c = preg_replace('/\\[js:(.*?)\\]/i', './t/' . $match2[1], $c);
            /*替换JS路径*/
            if ($page == 'm') {
                /*生成文章列表*/
                $poststr = '';
                if (file_exists('./../p/index.php')) {
                    ...
                    $recentin = $in;
                    $lastid = 0;
                    $clip = array_chunk($in, frontnum(), true);
                    /*分段文章*/
                    $clipnum = count($clip);
                    $in = $clip[$chsload];
                    /*先获取置顶文章列表*/
                    ...
                    if (!empty($in)) {
                        ...
                        foreach ($in as $key => $val) {
                            if (!in_array(intval($key), $tops, true)) {
                                /*排除置顶文章*/
                        ...
                        $c = preg_replace("/\t|\\[posts\\]/", $poststr, $c);
                        /*替换文章html*/
                    } else {
                        $result['result'] = 'notok';
                ...
                if (stripos($page, 'tag') !== false) {
                    ...
                    if (file_exists('./../p/index.php')) {
                        ...
                        if (empty($hand[1])) {
                            ...
                            $c = preg_replace("/\t|\\[tags\\]/", $poststr, $c);
                            /*替换标签页面html*/
                            $c = preg_replace("/\t|\\标签页/", '标签：' . $rt, $c);
                            /*替换标签头html*/
                        }
                    } else {
                        $result['result'] = 'notok';
